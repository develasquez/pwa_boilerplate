...
(function() {
	...
	monomer = {
		...
		pageShow: function(page, shp, _fun) {
		...
		__touch: {
			...
			removeTouch: function(event) {
				...
				_touch = $("._touch");
				_touch.css({
					top: event.changedTouches[0].clientY - 32,
					left: event.changedTouches[0].clientX - 32,
					display: "block"
				});
		...
		__expandFooter: function() {
			...
			setTimeout(function() {
				if ($(".footer").hasClass("footer-expanded")) {
					...
					$(".footer-expanded").css({
						"margin-top": -(_content.height - 2),
						"max-height": _content.height + em * 4
					})
		...
		__expandConfig: function(element, event) {
			...
			element.css({
				...
				"left": event.clientX - $(element).width() - 5
			});
		...
		__centerOverlay: function(_window, _content) {
			...
			$(".overlay").css({
				"top": parseInt(halfH - heihtOverlay)
			})
		},
		...
		__setInputLabelAspect: function(el) {
			var content = "";
			if ($(el).is("select")) {
				content = $(el).find("option").not(function() {
		...
		__setAspect: function() {
			...
			$(".loading").css({
				"top": (window.innerHeight / 2) - em,
				"left": (window.innerWidth / 2) - em
			});
			...
			$("input, select").change();
			$(".floating-up-left").css({
				"top": _header.height + (em * 2),
				"left": 0
			});
			$(".floating-up-right").css({
				"top": _header.height + (em * 2),
				"left": _window.width - (em * 2) - 56
			});
			$(".floating-down-left").css({
				"top": _window.height - _footer.height - (em * 4) - 56,
				"left": 0
			});
			$(".floating-down-right").css({
				"top": _window.height - _footer.height - _tabBar.height - _header.height - (em * 4) - 43,
				"left": _window.width - (em * 2) - 56
			});

			$(".floating-down-right-multiple.collapsed").css({
				"top": parseInt($(".floating-down-right-multiple.collapsed").parent().css("height")) - (em * 3),
				"left": $(".floating-down-right-multiple.collapsed").parent().width() - (em * 5)
			});
			$(".floating-down-right-multiple.expanded").css({
				"top": parseInt($(".floating-down-right-multiple.expanded").parent().css("height")) - (em * 8),
				"left": $(".floating-down-right-multiple.expanded").parent().width() - (em * 9)
			});
		...
		__init: function() {
			...
			$(".floating-down-right-multiple .action").on("click", function() {
				...
				if ($(fDRMultiple).hasClass("collapsed")) {
					...
					$(fDRMultiple).css({
						"top": parseInt($(parentContent).css("height")) - (em * 8),
						"left": $(parentContent).width() - (em * 9)
					});
				} else {
					...
					$(fDRMultiple).css({
						"top": parseInt($(parentContent).css("height")) - (em * 3),
						"left": $(parentContent).width() - (em * 5)
					});
				}
	...
	$(function() {
		...
		$("body").append($("<div>").addClass("hidden loading icon-spinner icon-2x"));
		...
		$(".tabBar").each(function(iT, eT) {
			...
			var widthTab = ((98 / allLi.length || 1)).toString() + "%";
			var widthSelected = ((99.9 / allLi.length || 1)).toString() + "%";
			allLi.css("width", widthTab);
			selectedTab.css("width", widthSelected);
		})
		$(".tabBar li.tab").on("click", function() {
			...
			ul.find(".selectedTab").css("margin-left", parseFloat($(".selectedTab").css("width")) * li.index() + "px")
